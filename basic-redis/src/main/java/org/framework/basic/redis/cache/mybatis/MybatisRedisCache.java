...
import org.framework.common.util.SerializeUtils;
...
//        Iterator<Jedis> iterator = CachePool.getInstance().getJedis().getAllShards().iterator();
//        Long dbSize = 0l;
...
//        CachePool.getInstance().getJedis().set(SerializeUtil.serialize(key.toString()), SerializeUtil.serialize(value));
    }
    ...
    public Object getObject(final Object key) {
        Object value = execute(new RedisCallback() {
            @Override
            public Object doWithRedis(ShardedJedis jedis) {
                return SerializeUtils.unserialize(jedis.hget(id.toString().getBytes(), key.toString().getBytes()));
            }
        });
//        Object value = SerializeUtil.unserialize(CachePool.getInstance().getJedis().get(SerializeUtil.serialize(key.toString())));
...
//        return CachePool.getInstance().getJedis().expire(SerializeUtil.serialize(key.toString()), 0);
    }
